...
use libsql::params;
use uuid::Uuid;
use yams_core::models::{Animal, NewAnimal, NewRace, Race};
...
impl AnimalRepository for SqliteAdapter {
    async fn find_all(&self) -> Result<Vec<Animal>, yams_core::Error> {
        let mut rows = self
            .db
            .query("SELECT id, name, birthdate, race_id FROM animals", ())
            .await
    ...
    async fn delete(&self, id: Uuid) -> Result<(), yams_core::Error> {
        self.db
            .execute(
                "DELETE FROM client_animals WHERE animal_id = ?1",
                params![id.to_string()],
            )
            .await
            .map_err(|e| yams_core::Error::Database(e.to_string()))?;
        self.db
            .execute("DELETE FROM animals WHERE id = ?1", params![id.to_string()])
            .await
            .map_err(|e| yams_core::Error::Database(e.to_string()))?;
        Ok(())
    }
...
impl RaceRepository for SqliteAdapter {
    async fn find_all(&self) -> Result<Vec<Race>, yams_core::Error> {
        let mut rows = self
            .db
            .query("SELECT id, description, animal_species FROM races", ())
            .await
            .map_err(|e| yams_core::Error::Database(e.to_string()))?;
    ...
    async fn delete(&self, id: Uuid) -> Result<(), yams_core::Error> {
        ...
        let mut rows = self
            .db
            .query(
                "SELECT count(*) FROM animals WHERE race_id = ?1",
                params![id.to_string()],
            )
            .await
            .map_err(|e| yams_core::Error::Database(e.to_string()))?;
        ...
        if count > 0 {
            return Err(yams_core::Error::Validation(
                "Cannot delete race used by animals".to_string(),
            ));
        }

        self.db
            .execute("DELETE FROM races WHERE id = ?1", params![id.to_string()])
            .await
            .map_err(|e| yams_core::Error::Database(e.to_string()))?;
        Ok(())
    }
...
fn map_row_to_animal(row: &libsql::Row) -> Result<Animal, yams_core::Error> {
    ...
    Ok(Animal {
        ...
        birthdate: DateTime::parse_from_rfc3339(&birthdate_str)
            .map_err(|e| yams_core::Error::Database(e.to_string()))?
            .with_timezone(&Utc),
        race_id: Uuid::parse_str(&race_id_str)
            .map_err(|e| yams_core::Error::Database(e.to_string()))?,
    })
}
