...
use yams_core::models::{Animal, Race};
...
impl AnimalRepository for SqliteAdapter {
    ...
    async fn save(&self, animal: Animal) -> Result<Animal, yams_core::Error> {
        let id = animal.id.unwrap_or_else(Uuid::new_v4);
...
impl RaceRepository for SqliteAdapter {
    ...
    async fn save(&self, race: Race) -> Result<Race, yams_core::Error> {
        let id = race.id.unwrap_or_else(Uuid::new_v4);
