...
impl ClientRepository for SqliteAdapter {
    ...
    async fn update(&self, client: Client) -> Result<Client, yams_core::Error> {
        ...
        self.db
            .execute(
                "DELETE FROM client_animals WHERE client_id = ?1",
...
impl SqliteAdapter {
    pub(crate) async fn get_client_animals(
        &self,
        client_id: Uuid,
    ) -> Result<Vec<Uuid>, yams_core::Error> {
        let mut rows = self
            .db
            .query(
            ...
            .map_err(|e| yams_core::Error::Database(e.to_string()))?
        {
            let id_str: String = row
                ...
                .map_err(|e| yams_core::Error::Database(e.to_string()))?;
            animal_ids.push(
                Uuid::parse_str(&id_str).map_err(|e| yams_core::Error::Database(e.to_string()))?,
            );
        }
        Ok(animal_ids)
    }
...
fn map_row_to_client(row: &libsql::Row) -> Result<Client, yams_core::Error> {
    ...
    Ok(Client {
        ...
        customer_number: row
            .get(6)
            .map_err(|e| yams_core::Error::Database(e.to_string()))?,
        address_id: Uuid::parse_str(&address_id_str)
            .map_err(|e| yams_core::Error::Database(e.to_string()))?,
        consent: row
            .get(8)
            .map_err(|e| yams_core::Error::Database(e.to_string()))?,
