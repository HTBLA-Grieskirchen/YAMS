...
use std::str::FromStr;
use uuid::Uuid;
use yams_core::models::{Event, NewEvent, NewSeminar, Seminar};
...
impl SeminarRepository for SqliteAdapter {
    async fn find_all(&self) -> Result<Vec<Seminar>, yams_core::Error> {
        let mut rows = self
            .db
            .query("SELECT id, title, price, duration FROM seminars", ())
            .await
            .map_err(|e| yams_core::Error::Database(e.to_string()))?;
    ...
    async fn delete(&self, id: Uuid) -> Result<(), yams_core::Error> {
        ...
        let mut rows = self
            .db
            .query(
                "SELECT count(*) FROM events WHERE seminar_id = ?1",
                params![id.to_string()],
            )
            .await
            .map_err(|e| yams_core::Error::Database(e.to_string()))?;
        ...
        if count > 0 {
            return Err(yams_core::Error::Validation(
                "Cannot delete seminar used by events".to_string(),
            ));
        }

        self.db
            .execute(
                "DELETE FROM seminars WHERE id = ?1",
                params![id.to_string()],
            )
            .await
            .map_err(|e| yams_core::Error::Database(e.to_string()))?;
...
fn map_row_to_event(row: &libsql::Row) -> Result<Event, yams_core::Error> {
    ...
    Ok(Event {
        ...
        date: DateTime::parse_from_rfc3339(&date_str)
            .map_err(|e| yams_core::Error::Database(e.to_string()))?
            .with_timezone(&Utc),
        location_id: Uuid::parse_str(&location_id_str)
            .map_err(|e| yams_core::Error::Database(e.to_string()))?,
        location_name: row
            .get(3)
            .map_err(|e| yams_core::Error::Database(e.to_string()))?,
        max_participants: row
            .get(4)
            .map_err(|e| yams_core::Error::Database(e.to_string()))?,
        seminar_id: Uuid::parse_str(&seminar_id_str)
            .map_err(|e| yams_core::Error::Database(e.to_string()))?,
    })
}
...
fn map_row_to_seminar(row: &libsql::Row) -> Result<Seminar, yams_core::Error> {
    ...
    let price_str: String = row
        .get(2)
        .map_err(|e| yams_core::Error::Database(e.to_string()))?;
    let duration_ms_str: Option<String> = row
        .get(3)
        .map_err(|e| yams_core::Error::Database(e.to_string()))?;
