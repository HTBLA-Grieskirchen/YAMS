...
async fn main() -> Result<(), std::io::Error> {
    ...
    let ctx = Arc::new(YamsContext::new(
        adapter.clone(), // address_repo
        adapter.clone(), // client_repo
        adapter.clone(), // animal_repo
        adapter.clone(), // race_repo
        adapter.clone(), // event_repo
        adapter.clone(), // seminar_repo
    ));

    let address_service = AddressService::new(ctx.clone());

    let api_service = OpenApiService::new(api::Api { address_service }, "YAMS API", "1.0")
