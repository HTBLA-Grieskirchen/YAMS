...
fn into_structured_error_from_frame(frame: &Frame, head: &[&Frame]) -> StructuredError {
...
impl<E: StableError> From<Report<E>> for StructuredError {
    fn from(value: Report<E>) -> Self {
        // Top-level sources: flatten/collect
        let error: StructuredError = into_structured_error_from_frame(value.current_frame(), &[]);
