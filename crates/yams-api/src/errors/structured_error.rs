...
impl<E: ThreadSafeError> From<Report<E>> for StructuredError {
    fn from(value: Report<E>) -> Self {
        ...
        let error: Vec<StructuredError> = into_structured_error_from_frame(value.current_frame(), &[]);

        // Should always yield one at top level. Fall back to dummy if not.
        error.into_iter().next().expect("Expected at least one top level error context in report")
    }
}
