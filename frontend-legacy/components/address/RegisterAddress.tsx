...
function clientRegisterAddressData(
  ...
  return {
    ...
    street: new ValidatableFieldData<string>(initialStreet, (value) => {
      if (value.trim().length < 1) {
    ...
    streetNumber: new ValidatableFieldData<string>(
      initialStreetNumber,
      (value) => {
        if (value.trim().length < 1) {
...
export function clientRegisterAddressDataFromAddress(
  address: Address,
): NewAddressFormData {
  return clientRegisterAddressData(
    address.postalCode,
    address.city,
    address.country,
    address.street,
...
const RegisterAddressForm = observer(
  ({ addressData }: { addressData: NewAddressFormData }) => {
    return (
      <>
