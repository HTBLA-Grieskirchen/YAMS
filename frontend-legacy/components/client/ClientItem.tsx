      ...
      <div className="flex flex-col w-fit">
        <div className="flex flex-row space-x-4 items-center">
          ...
          <div className="flex flex-row h-full items-center space-x-1">
            ...
            <div className="dropdown dropdown-end">
              ...
              <ul
                tabIndex={0}
                className="dropdown-content menu p-2 shadow-lg bg-base-200 rounded-btn w-52"
              >
                <li>
        ...
        {showDetail ? (
          <div className="bg-white rounded-xl p-2 w-[42rem] mt-2">
            <p className="text-xs text-gray-400">FURTHER INFORMATION:</p>
            <div className="flex flex-col mb-1 w-fit">
              <a
                className="font-medium text-lg min-w-full xl:max-w-4xl sm:max-w-sm max-w-0 truncate underline decoration-blue-700 text-blue-700 hover:no-underline hover:text-blue-600"
            ...
            <div className="flex flex-col mb-1">
              <p className="text-base text-gray-600 min-w-full xl:max-w-4xl sm:max-w-sm max-w-0 truncate">
                ...
                {client.mobileNumber.at(0) == "+" ? (
                  <>
                    {client.mobileNumber.substring(0, 3) +
                      " " +
                      client.mobileNumber.substring(3, 6) +
                      " " +
                      client.mobileNumber.substring(6, 13)}
                  </>
                ) : (
                  <>
                    {client.mobileNumber.substring(0, 4) +
                      " " +
                      client.mobileNumber.substring(
                        4,
                        client.mobileNumber.length,
                      )}
                  </>
                )}
              </p>
            ...
            <p className="text-xs text-gray-400 mb-1">REGISTERED ANIMALS:</p>
            {deleteSubmitted ? <></> : <AnimalList client={client} />}
          </div>
        ) : (
          <div></div>
        )}
