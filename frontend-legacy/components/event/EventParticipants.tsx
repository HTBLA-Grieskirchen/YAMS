...
const EventParticipants = observer(({ event }: { event: Event }) => {
  ...
  const filteredParticipants = participants.filter((participation) =>
    participationSearchedClient(participation, participantFilter.value),
  );
...
function participationSearchedClient(
  ...
  return query
    ...
    .every((segment) => {
      ...
      return (
        client.firstName.toLowerCase().includes(trimmedSegment) ||
