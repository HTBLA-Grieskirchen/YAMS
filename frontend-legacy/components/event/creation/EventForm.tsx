...
import RegisterSeminarForm, {
  defaultSeminarFormData,
  NewSeminarFormData,
} from "../../seminar/RegisterSeminar";
...
const EventForm = observer(({ event }: { event?: Event }) => {
  ...
  const maxParticipants = useLocalObservable(
    () =>
      new ValidatableFieldData<number | null>(
  ...
  const location = useLocalObservable(
    () =>
      new ValidatableFieldData<Address | NewAddressFormData | null>(
        event?.location ?? null,
        (value) => {
          if (value == null) {
  ...
  const seminar = useLocalObservable(
    () =>
      new ValidatableFieldData<Seminar | NewSeminarFormData | null>(
        event?.seminar ?? null,
        (value) => {
          if (value == null) {
  ...
  function anyMutated() {
    let mutated = [
      ...
      date,
      time,
    ].some((data) => data.modified);
    mutated =
      mutated ||
      (location.value == newAddress &&
        Object.values(newAddress).some((data) => data.modified));
    mutated =
      mutated ||
      (location.value == newAddress &&
        Object.values(newAddress).some((data) => data.modified));
  ...
  function allValid() {
    let valid = [
      ...
      date,
      time,
    ].every((data) => data.valid);
    valid =
      valid &&
      (location.value != newAddress ||
        Object.values(newAddress).every((data) => data.valid));
    valid =
      valid &&
      (seminar.value != newSeminar ||
        Object.values(newSeminar).every((data) => data.valid));
    return valid;
  }

  const newAddress = useLocalObservable(
    () => emptyClientRegisterAddressFieldData,
  );
      ...
      : addresses.filter((address) => {
          return addressSearchQuery
            ...
            .every((segment) => {
              ...
              return (
                address.city.toLowerCase().includes(trimmedSegment) ||
                ...
                (address.extra?.toLowerCase()?.includes(trimmedSegment) ??
                  false) ||
  ...
  const filteredSeminars =
    seminarSearchQuery.trim().length < 1
      ? seminars
      : seminars.filter((seminar) => {
          return seminarSearchQuery
            .trim()
            .split(" ")
            .flatMap((item) => item.split(","))
            .every((segment) => {
              ...
              return (
                seminar.title.toLowerCase().includes(trimmedSegment) ||
                seminar.price
                  .toString()
                  .toLowerCase()
                  .includes(trimmedSegment) ||
                formatDuration(seminar.duration ?? 0, language)
                  .toLowerCase()
                  .includes(trimmedSegment)
              );
            });
    ...
    if (anyMutated()) {
      dialog((close) => (
        <div className="modal-box">
          ...
          <p className="py-4">
            Are you sure you want to abort the event{" "}
            {!event ? "creation" : "modification"}
            and return to the previous view? All data will be lost!
          </p>
          <div className="modal-action">
            <button className="btn btn-neutral" onClick={(e) => close()}>
              Stay here
            </button>
            <button
  ...
  function submit() {
    const data = {
      ...
      formData: {
        ...
        location:
          location.value instanceof Address
            ? location.value
            : {
        ...
        seminar:
          seminar.value instanceof Seminar
            ? seminar.value
            : {
    ...
    async function transact() {
      ...
      try {
        ...
        const clientRecord = makeRecordForTable(
    ...
    <>
      ...
      >
        <div className="flex justify-between space-x-4">
          ...
          <ValidatableInputField
            ...
            label="Max allowed participants"
            placeholder="15"
            className="basis-96 shrink grow-0"
            mapDisplayValue={(val) => (val == null ? "" : val.toString())}
            mapSetValue={(val) => {
              const value =
                val.trim().length <= 0 ? null : Number.parseInt(val);
        ...
        {seminar.value == newSeminar && (
          <RegisterSeminarForm seminarData={newSeminar} />
        )}
        ...
        <div className="flex space-x-4">
          <ValidatableInputField
            data={locationName}
            label="Location name"
            placeholder="Medical University Linz"
            className="basis-96 shrink grow-0"
          />
        ...
        {location.value == newAddress && (
          <RegisterAddressForm addressData={newAddress} />
        )}
      </form>
      ...
      <div className="card-actions justify-end">
        <button
          ...
          onClick={(e) => onBack()}
        >
          <i className="fa-solid fa-circle-left mr-2" />
          Back
        </button>
        <button
          className={`btn btn-success ${form.submitted ? "loading" : ""}`}
          type="submit"
        ...
        >
          {!form.submitted && (
            <i
              className={`fa-solid ${!event ? "fa-calendar-plus" : "fa-pen-to-square"} mr-2`}
            />
          )}
          {!event ? "Create" : "Change"}
        </button>
