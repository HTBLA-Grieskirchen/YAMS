...
export function askAddParticipantEvent(
  event: Event,
  state?: SubmissionState,
  callback?: () => Promise<void>,
) {
  const DialogComponent = observer(({ close }: { close: () => void }) => {
    const store = useStore();
    const participants =
      store.eventStore.participationStore.indexedByDest.get(
        event.record.join(),
      ) ?? [];
    const potentialClients = store.clientStore.clients.filter((client) =>
      participants.every((participation) => participation.from != client),
    );
      ...
      >
        ...
        <div className="flex grow space-x-4 py-4">
          ...
          <ValidatableInputField
            data={price}
            label="Price"
            placeholder={event.seminar.price.toString()}
            className="basis-48 shrink grow-0"
            required
            mapDisplayValue={(num) =>
              price.modified ? undefined : num.toString()
            }
            mapSetValue={(val) =>
              val.trim().length < 1 ? Number.NaN : Number(val)
            }
          />
        </div>
        <div className="modal-action">
          ...
          <button
            className={`btn btn-success ${!!state?.submitted ? "loading" : ""}`}
            type="submit"
...
export async function submitAddParticipantEvent(
  event: Event,
  client: Client,
  cost: number,
  state?: SubmissionState,
  ...
  const result = await relateClientParticipateEvent(
    client.record,
    event.record,
    cost.toString(),
  );
...
export function askEditEventParticipation(
  ...
  const DialogComponent = observer(({ close }: { close: () => void }) => {
    ...
    return (
      <form
        ...
        onSubmit={(e) => {
          e.preventDefault();
          submitUpdateEventParticipation(
            participation,
            price.value,
            state,
            callback,
      ...
      >
        <h3 className="font-bold text-lg">Change participation...</h3>
        <div className="flex grow space-x-4 py-4">
          <div className={`form-control w-full max-w-md basis-96`}>
            ...
            <div className="indicator w-full">
              <p
                className={`ml-1 mt-2 text-lg font-bold w-full pointer-events-none`}
              >
...
export async function submitUpdateEventParticipation(
  participation: EventParticipation,
  cost: number,
  state?: SubmissionState,
  callback?: () => Promise<void>,
...
export async function submitDeleteEventParticipation(
  participation: EventParticipation,
  state?: SubmissionState,
  callback?: () => Promise<void>,
) {
