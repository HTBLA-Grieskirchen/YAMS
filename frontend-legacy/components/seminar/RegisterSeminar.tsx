...
const RegisterSeminarForm = observer(
  ({ seminarData }: { seminarData: NewSeminarFormData }) => {
    return (
      <div className="flex space-x-4">
        <ValidatableInputField
          data={seminarData.title}
          label="Title"
          placeholder="Spine Injury Treatment"
          className="max-w-lg shrink grow-0"
        />
        <ValidatableInputField
          data={seminarData.duration}
          label="Duration"
          placeholder="2h 15m"
          className="basis-96 shrink grow-0"
          mapDisplayValue={(_) => undefined}
          mapSetValue={(val) =>
            val.trim().length < 1 ? null : (parseDuration(val) ?? Number.NaN)
          }
        />
        <ValidatableInputField
          data={seminarData.price}
          label="Price"
          placeholder="20.50"
          className="basis-96 shrink grow-0"
          mapDisplayValue={(_) => undefined}
