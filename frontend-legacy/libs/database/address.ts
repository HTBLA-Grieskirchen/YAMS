...
export async function deleteAddress(address: Address): Promise<Result<any>> {
  const checkResult = await query(
    "SELECT id FROM client, event WHERE address = type::thing($addressTable, $addressID)",
    {
  ...
  const response = await query(
    "DELETE type::thing($addressTable, $addressID)",
    {
      addressTable: address.record.table,
      addressID: address.record.id,
    },
...
export async function patchAddressesDynamic(
  prev: { [field: string]: string },
  next: { [field: string]: string },
): Promise<Result<any>> {
  const prefixedPrev = Object.entries(prev).map(([field, value]) => [
    `prev_${field}`,
    value,
  ]);
  const prefixedNext = Object.entries(next).map(([field, value]) => [
    `next_${field}`,
    value,
  ]);

  const prevRequirements = Object.keys(prev)
    .map((field, index) => `${field} = $${prefixedPrev[index][0]}`)
    .join(" AND ");
  const nextValues = Object.keys(next)
    .map((field, index) => `${field} = $${prefixedNext[index][0]}`)
    .join(", ");
  ...
  const response = await query(
    ...
    `,
    {
      ...Object.fromEntries(prefixedPrev),
      ...Object.fromEntries(prefixedNext),
      addressTable: Address.TABLE,
    },
  );
