...
export default class Event {
  ...
  get daysFromNow() {
    return (
      Math.round(
        new Date(this.date).setUTCHours(0, 0, 0) -
...
export class EventResponse implements DatabaseResponse<Event> {
  ...
  applyOn(object: Event): void {
    ...
    runInAction(() => {
      ...
      if (this.data.location_name != object.locationName)
        object.locationName = this.data.location_name;
      if (this.data.max_participants != object.maxParticipants)
        object.maxParticipants = this.data.max_participants;
    });
  }
