...
export class RaceResponse implements DatabaseResponse<Race> {
  ...
  applyOn(object: Race): void {
    ...
    runInAction(() => {
      if (this.data.description != object.description)
        object.description = this.data.description;
      if (this.data.animal_species != object.animal_species)
        object.animal_species = this.data.animal_species;
    });
  }
