...
const EventDetail: NavigationPage = observer(() => {
  ...
  if (
    typeof id !== "string" ||
    (event = store.eventStore.indexedEvents.get(id)) === undefined
  )
    return (
                      ...
                      {!!event.seminar.duration && (
                        <>
                          ...
                          {Math.floor(-timeRemaining / 60_000) == 420 ? (
                            ...
                            formatDuration(
                              event.seminar.duration + timeRemaining,
                              language,
                            )
                          )}
                        </>
...
EventDetail.NavMenu = observer(() => {
  ...
  if (
    typeof id !== "string" ||
    (temp_event = store.eventStore.indexedEvents.get(id)) === undefined
  )
    return <></>;
  ...
  return (
    <>
      <NavbarMenuEntry disabled={deleteState.submitted}>
        <a
          onClick={(e) =>
            askSubmitDeleteEvent(event, deleteState, async () => {
              await router.replace(paths.events);
            })
          }
        >
          <i className="fa-solid fa-users-slash text-error" />
          Cancel
        </a>
      </NavbarMenuEntry>
      <NavbarMenuEntry>
        <Link href={paths.event_edit(event.record.join())}>
          <a>
            <i className="fa-solid fa-pencil-alt text-primary" />
            Edit
          </a>
        </Link>
      </NavbarMenuEntry>
      <NavbarMenuEntry
        disabled={addParticipantState.submitted || !potentialClients.length}
      >
