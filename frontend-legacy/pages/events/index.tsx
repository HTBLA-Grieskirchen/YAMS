...
const EventsOverview: NavigationPage = observer(() => {
  ...
  const filteredEvents = store.eventStore.events.filter((event) =>
    eventSearched(event, filter),
  );
  const categorizedEvents =
    category == "upcoming"
      ? filteredEvents
          .filter((event) => showPast && event.when == 0)
          .sort((e1, e2) => e2.date.valueOf() - e1.date.valueOf())
          .slice(0, 4)
          .reverse()
          .concat(
            filteredEvents
              .filter((event) => event.when <= 2 && event.when > 0)
              .sort((e1, e2) => e1.date.valueOf() - e2.date.valueOf()),
          )
      : category == "future"
        ? filteredEvents
            .filter((event) => event.when > 0)
            .sort((e1, e2) => e1.date.valueOf() - e2.date.valueOf())
        : category == "past"
          ? filteredEvents
              .filter((event) => event.when == 0)
...
function eventSearched(event: Event, query: string): boolean {
  return query
    ...
    .every((segment) => {
      ...
      return (
        event.seminar.title.toLowerCase().includes(trimmedSegment) ||
