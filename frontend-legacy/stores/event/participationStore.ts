...
export default class ParticipationStore {
  ...
  private syncData() {
    ...
    runInAction(() => {
      ...
      const participationIDs: Set<string> = new Set(
        result[0].result
          .map((item: any) => {
            ...
            let indexFromArray =
              this.indexedByFrom.get(participation.from.record.join()) ??
              (() => {
            ...
            let indexDestArray =
              this.indexedByDest.get(participation.dest.record.join()) ??
              (() => {
                const arr = observable.array();
      ...
      Array.from(this.indexed.keys())
        .filter((id) => !participationIDs.has(id))
        .forEach((id) => this.indexed.delete(id));
    });
  }
}
