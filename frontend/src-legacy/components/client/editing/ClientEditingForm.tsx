...
const EditClientForm = observer(({ client }: { client: Client }) => {
  ...
  const newAddress = useLocalObservable(() =>
    clientRegisterAddressDataFromAddress(client.address),
  );
  ...
  const anyMutated = () => {
    let mutated = [firstname, lastname, mobilenumber, birthdate, email].some(
      (data) => data.modified,
    );
    mutated =
      mutated ||
    ...
    if (anyMutated()) {
      dialog((close) => (
        <div className="modal-box">
          ...
          <p className="py-4">
            Are you sure you want to abort the update-process and return to the
            previous view? All data will be lost!
          </p>
  ...
  async function submit() {
    ...
    if (address.value == newAddress) {
      const potentialDuplicate = store.addressStore.addresses.find((item) => {
        return (
          (item.extra ?? "") == newAddress.extra.value.trim() &&
      ...
      if (potentialDuplicate !== undefined) {
        ...
        try {
          addressRecord = makeRecordForTable(
  ...
  return (
    <>
      ...
      >
        <div className="flex justify-between space-x-4">
          ...
          <ValidatableInputField
            ...
            mapDisplayValue={(value) =>
              value == null ? "" : value.toISOString().split("T")[0]
            }
            mapSetValue={(value) => {
        ...
        {address.value != null && !(address.value instanceof Address) && (
          <ClientRegisterAddressForm addressData={address.value} />
        )}
      </form>
