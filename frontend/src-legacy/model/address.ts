...
export class AddressResponse implements DatabaseResponse<Address> {
  ...
  applyOn(object: Address): void {
    ...
    runInAction(() => {
      if (this.data.country != object.country)
        object.country = this.data.country;
      if (this.data.postal_code != object.postalCode)
        object.postalCode = this.data.postal_code;
      if (this.data.city != object.city) object.city = this.data.city;
      if (this.data.street != object.street) object.street = this.data.street;
      if (this.data.street_number != object.streetNumber)
        object.streetNumber = this.data.street_number;
      if (this.data.street != object.street) object.street = this.data.street;
