...
export const AddressTableHeader = observer(
  ({ selectedCategories }: { selectedCategories: Map<string, boolean> }) => {
    return (
      <tr>
...
export const AddressTableRow = observer(
  ({
    ...
    facingValues: { [key: string]: string };
    backingAddresses: Address[];
  }) => {
    ...
    const usingClients = store.clientStore.clients.filter((client) =>
      backingAddresses.find(
        (address) => client.address.record.id === address.id,
      ),
    );
    const usingEvents = store.eventStore.events.filter((event) =>
      backingAddresses.find(
        (address) => event.location.record.id === address.id,
      ),
    );
    ...
    return (
      <>
        ...
        <tr>
          <td colSpan={100}>
            <div
              className={`collapse ${expanded ? "collapse-open" : "collapse-close"}`}
            >
              <div
                className={`collapse-title text-xl font-normal p-0 ${expanded ? "-mb-2" : "-mb-6"}`}
              >
                <button
                  className={`btn btn-circle btn-ghost btn-sm mr-2 ${usages <= 0 && "invisible"}`}
                  onClick={(e) => setExpanded(!expanded)}
                >
                  <i
                    className={`fa-solid fa-chevron-down transition-transform ${expanded ? "rotate-180" : "rotate-0"}`}
                  />
                </button>
      ...
      <>
        ...
        <NoBorderTD>
          <div className="relative flex place-content-end">
            ...
            <EditButton
              entries={usageAmount}
              onClick={() => runInAction(() => (editState.editing = true))}
            />
          </div>
        </NoBorderTD>
      </>
  ...
  ({
      ...
      if (usageAmount > 1) {
        dialog((close) => (
          <div className="modal-box">
            ...
            <p className="py-4">
              Are you sure you want to update this many items? All addresses in
              this category will be changed.
            </p>
            <div className="modal-action">
              <button className="btn btn-error" onClick={(e) => close()}>
                Cancel
              </button>
              <button
    ...
    async function submit() {
      ...
      const prev = Object.fromEntries(
        Object.entries(facingValues).map(([field, value]) => [
          categories[field].databaseField,
          value,
        ]),
      );
      const next = Object.fromEntries(
        Object.entries(editValue).map(([field, value]) => [
          categories[field].databaseField,
          value,
        ]),
      );
      ...
      const result = {
        error: { message: "Update not yet implemented in new architecture" },
      };
    ...
    return (
      <>
        {Object.entries(categories).map(
          ([field, { humanReadable, validation }]) => {
            return (
              <NoBorderTD key={field}>
                {selectedCategories.get(field) ? (
                  <div className="form-control w-full max-w-xs">
                    <input
                      type="text"
                      form={id}
                      placeholder={`Type ${humanReadable.toLowerCase()}`}
                      value={(editValue as any)[field]}
                      onChange={(e) => {
                        runInAction(() => {
                          errors.set(
                            field,
                            validation ? validation(e.target.value) : null,
                          );
        ...
        <NoBorderTD>
          <div className="relative">
            <EditButtons
              id={id}
              invalid={
                !Array.from(errors.values()).every((error) => error == null)
              }
              onConfirm={() => askSubmit()}
              onCancel={() => runInAction(() => (editState.editing = false))}
...
const EditButtons = observer(
  ({
    ...
    return (
      <form
        id={id}
        className="flex flex-row space-x-10 place-content-end"
        onSubmit={(e) => {
          e.preventDefault();
          onConfirm && onConfirm();
        }}
...
const EditButton = observer(
  ({
    ...
    return (
      <button
        className={`absolute top-0 right-0 btn btn-sm ${hidden ? "invisible" : ""}`}
...
const AddressUsageInfo = observer(
  ({ clients, events }: { clients: Client[]; events: Event[] }) => {
    type Tabs = "clients" | "events";
    const [selectedTab, setSelectedTab] = useState<Tabs>(
      !clients.length ? "events" : "clients",
    );
      ...
      <div className="grid">
        ...
        <div className="w-full max-h-72 relative overflow-auto">
          <div className="bg-base-200 rounded-b-box rounded-tr-box px-4 py-2 shadow-lg">
            {selectedTab == "clients" ? (
              <ClientUsages clients={clients} />
            ) : selectedTab == "events" ? (
              <EventsUsages events={events} />
            ) : (
              <></>
            )}
          </div>
...
const ClientUsages = observer(({ clients }: { clients: Client[] }) => {
  return (
    <div className="flex flex-col divide-y divide-base-300 w-full">
      {clients.map((client) => {
        return (
          <div
            key={client.record.join()}
            className="w-full flex place-content-between"
          >
            <div className="flex flex-col my-1">
              <div className="flex flex-row">
                <p className="text-lg font-medium mb-1 mr-2">
              ...
              <div className="flex flex-row">
                ...
                <div
                  className="tooltip tooltip-accent tooltip-right w-fit cursor-help"
...
const EventsUsages = observer(({ events }: { events: Event[] }) => {
  const store = useStore();
    ...
    <div className="flex flex-col divide-y divide-base-300 w-full">
      {events.map((event) => {
        ...
        const dayDifference = -Math.round(
          Math.abs(
            new Date(event.date).setUTCHours(0, 0, 0) -
            ...
            <div className="flex flex-col my-1">
              ...
              <div className="flex flex-row">
                <span>
                  ...
                  {dateFormat} at{" "}
                  {event.date.toLocaleTimeString(language, {
                    timeStyle: "short",
                  })}
                  {!!event.seminar.duration && (
                    <span className="ml-4">
                      <i className="fa-solid fa-clock mr-2" />
                      {formatDuration(event.seminar.duration, language)}
                    </span>
                  )}
                </span>
