...
const EventOverviewItem = observer(({ event }: { event: Event }) => {
  ...
  return (
    <div className="py-4 first:pt-0 last:pb-0 flex flex-row justify-between group">
      <div>
        <h3 className="mb-3">
          ...
          <span className="mx-4 text-base-content/75">
            ...
            {participants.length}{" "}
            {event.maxParticipants != null && ` of ${event.maxParticipants}`}
          </span>
          {eventWhen <= 0 ? (
        ...
        <div className="text-base-content/75 flex flex-row">
          <span>
            ...
            {dateFormat} at{" "}
            {event.date.toLocaleTimeString(language, { timeStyle: "short" })}
            {!!event.seminar.duration && (
              <span className="ml-4 text-base-content/75">
                <i className="fa-solid fa-hourglass-end mr-2" />
      ...
      <div className="self-center dropdown dropdown-left group-last:dropdown-end group-last:dropdown-left">
        ...
        <ul
          tabIndex={0}
          className="dropdown-content menu menu-compact shadow-lg bg-base-100 rounded-box w-52"
        >
          <li>
          ...
          <li>
            <a
              onClick={(e) => askSubmitDeleteEvent(event, deleteState)}
              className={
                !deleteState.submitted
                  ? ""
                  : "pointer-events-none bg-base-200/50 opacity-50"
              }
            >
              {deleteState.submitted ? (
