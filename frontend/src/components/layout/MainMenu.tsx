...
import { Button, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, Card, CardBody } from "@heroui/react";
...
const MainMenu = observer((
    ...
    return <div className="flex h-screen overflow-hidden">
        ...
        {/* Overlay */}
        <label htmlFor="main-drawer" className="fixed inset-0 z-40 bg-black/50 transition-opacity opacity-0 pointer-events-none peer-checked:opacity-100 peer-checked:pointer-events-auto lg:hidden" />
...
const MainMenuCategory = observer((
    {category, items}:
        { category: string, items: { [name: string]: MainMenuItemData } }
) => {
...
const MainMenuLogo = observer(() => {
    return <div className="px-2">
        <img src="/energetik_logo.png" className="w-12 h-12 mb-2" alt="Energetik Sabine Petschl"/>
        <div className="text-xs text-foreground-500 leading-tight">
            <p className="font-semibold text-foreground">Energetik Sabine Petschl</p>
    ...
    return (
        <Dropdown>
            ...
            <DropdownMenu 
                ...
                onAction={(key) => store.settingsStore.setTheme(key === "system" ? null : key as string)}
