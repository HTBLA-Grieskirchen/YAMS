...
const PurchaseUsageTabs = observer((
    {purchase}:
        { purchase: Purchase }
) => {
    ...
    return <div className="flex flex-col">
        ...
        <div className={`transition-all ${expanded ? "h-fit" : "h-0"} overflow-clip`}>
            <Tabs value={"client"} id={`${purchase.record.join()}_usages`} className="mt-2">
                ...
                <TabsBody>
                    <TabPanel value={"client"}>
                        <ClientContent purchase={purchase}/>
                    </TabPanel>
                    <TabPanel value={"event"}>
                        <EventContent purchase={purchase}/>
                    </TabPanel>
                </TabsBody>
            </Tabs>
        </div>
...
export default PurchaseUsageTabs
