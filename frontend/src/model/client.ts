...
export class ClientResponse implements DatabaseResponse<Client> {
  ...
  static from(item: any): ClientResponse | undefined {
    if (
      ...
      item.mobile_number === undefined ||
      item.address === undefined ||
      item.consent === undefined ||
      item.customer_number === undefined
    )
      return;
  ...
  applyOn(object: Client): void {
    ...
    runInAction(() => {
      if (this.data.address != object.address.record.join()) {
        const address = store.addressStore.indexedAddresses.get(
          this.data.address,
        );
        if (address) object.address = address;
      }

      if (this.data.first_name != object.firstName)
        object.firstName = this.data.first_name;
      if (this.data.last_name != object.lastName)
        object.lastName = this.data.last_name;
      if (this.data.email != object.email) object.email = this.data.email;
      if (this.data.mobile_number != object.mobileNumber)
        object.mobileNumber = this.data.mobile_number;
      if (this.data.customer_number != object.customerNumber)
        object.customerNumber = this.data.customer_number;
      if (this.data.consent != object.consent)
        object.consent = this.data.consent;
