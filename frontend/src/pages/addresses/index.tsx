...
import {
  AddressTableHeader,
  AddressTableRow,
} from "../../components/address/AddressTable";
...
export const categories: {
  ...
  streetNumber: {
    humanReadable: "Street Number",
    databaseField: "streetNumber",
    validation: (val) => {
    ...
    return (
      <div className="form-control">
        <label className="label cursor-pointer">
          ...
          <input
            type="checkbox"
            checked={selectedCategories.get(field)}
            className="checkbox checkbox-primary"
            onChange={(e) =>
              runInAction(() =>
                selectedCategories.set(field, !selectedCategories.get(field)),
              )
            }
          />
        </label>
      ...
      <main className="p-6 overflow-y-auto">
        <div className="card card-compact bg-base-100 shadow p-4 max-h-full">
          ...
          <div className="overflow-x-auto w-full mt-2">
            {isLoading ? (
            ...
            ) : !Array.from(selectedCategories.values()).reduce(
                (prev, cur) => prev || cur,
                false,
              ) ? (
              <div className="alert alert-warning">
                <div>
                  <i className="fa-solid fa-warning flex-shrink-0 text-xl" /> No
                  category selected!
                </div>
              </div>
            ) : !groups.length ? (
              <div className="alert alert-info shadow-lg">
                <div>
                  <i className="fa-solid fa-info-circle flex-shrink-0 text-xl" />{" "}
                  No addresses known.
                </div>
              </div>
            ) : (
