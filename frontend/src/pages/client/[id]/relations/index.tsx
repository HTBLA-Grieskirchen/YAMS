...
} from "../../../../libs/database/clientRelation";
...
const ClientRelations: NavigationPage = observer(() => {
  ...
  if (
    typeof id !== "string" ||
    (client = store.clientStore.indexedClients.get(id)) === undefined
  )
    return (
      <div className="p-6 overflow-y-auto h-full w-full flex flex-col items-center justify-center space-y-4">
  ...
  const relations =
    store.clientStore.relationStore.indexedByFrom.get(client.record.join()) ??
    [];
  const relationsFiltered = relations.filter((relation) =>
    clientSearched(relation.dest, filter.value),
  );
  ...
  return (
    <>
      <Head>
        <title>
          Relations | {client.firstName} {client.lastName}
        </title>
      </Head>
            ...
            <div className="overflow-y-auto relative">
              {!relations.length ? (
                ...
                <div className="flex flex-col space-y-4">
                  {Object.entries(
                    groupBy(relationsFiltered, (relation) =>
                      relation.dest.record.join(),
                    ),
                  ).map(([group, values]) => (
                    <RelationsPerClient key={group} relations={values} />
                  ))}
                </div>
              )}
            </div>
...
ClientRelations.NavMenu = observer(() => {
  ...
  return (
    <>
      {store.clientStore.clients.length > 1 && (
        <li>
          <a
            onClick={(e) =>
              dialog((close) => (
                <AddRelationDialog close={close} fromClient={client!} />
              ))
            }
          >
...
ClientRelations.NavPath = observer(() => {
  ...
  if (
    ...
    return <>{ClientDetail.NavPath && <ClientDetail.NavPath />}</>;
...
const AddRelationDialog = observer(
  ({ close, fromClient }: { close: () => void; fromClient: Client }) => {
    const [submitted, setSubmitted] = useState(false);

    const store = useStore();
    const relations = store.clientStore.relationStore;
    const potentialTargetClients = store.clientStore.clients.filter(
      (potential) => potential != fromClient,
    );
    const potentialRelationTypes = Array.from(
      new Set(
        relations.relations.map((relation) => relation.relationType),
      ).values(),
    );

    const typesFromTarget = () => {
      const outgoingRelations = relations.indexedByFrom.get(
        fromClient.record.join(),
      );
      return outgoingRelations === undefined
        ? []
        : outgoingRelations
            .filter((relation) => relation.dest == targetClient?.value)
      ...
      () =>
        new ValidatableFieldData("", (value) => {
          if (value.trim().length < 1) {
            ...
            typesFromTarget().some(
              (type) => type.toLowerCase().trim() == value.toLowerCase().trim(),
            )
          ) {
    ...
    const bidirectional = useLocalObservable(() => ({
      ...
      get isPossible() {
        const incomingRelations = relations.indexedByDest.get(
          fromClient.record.join(),
        );
        return (
          incomingRelations === undefined
            ? []
            : incomingRelations.filter(
                (relation) => relation.from == targetClient?.value,
              )
        ).every(
          (relation) =>
            relation.relationType.toLowerCase().trim() !=
            relationType.value.toLowerCase().trim(),
        );
      },
    }));

    const filteredTargetClients =
      targetClientQuery.trim().length < 1
        ? potentialTargetClients
        : potentialTargetClients.filter((client) => {
            return clientSearched(client, targetClientQuery);
          });
    const filteredRelationTypes =
      relationType.value.trim().length < 1
        ? potentialRelationTypes
    ...
    const addRelation = async () => {
      ...
      let result = await relateClients(
        fromClient.record,
        targetClient.value!.record,
        relationType.value,
      );
      if (await erroredResult()) return;

      if (bidirectional.value) {
        result = await relateClients(
          targetClient.value!.record,
          fromClient.record,
          relationType.value,
        );
    ...
    return (
      <form
        className="modal-box max-w-none w-fit overflow-visible"
        onSubmit={(e) => {
          e.preventDefault();
          addRelation().then();
        }}
      >
        <h3 className="font-bold text-lg">Add new relation...</h3>
        <div className="flex grow space-x-4 py-4">
          ...
          <ValidatableComboBox
            ...
            disabled={targetClient.value == null}
          ...
          <div className="form-control grow mt-8">
            <label
              className={`label ${bidirectional.isPossible ? "cursor-pointer" : "cursor-help"} w-full`}
            >
              <div
                className="label-text text-left tooltip tooltip-accent tooltip-top"
                data-tip="The relation will be created in both directions"
              >
                <span
                  className={`${!bidirectional.isPossible ? "opacity-50" : ""}`}
                >
        ...
        {!!typesFromTarget().length && (
          <div className="prose w-full">
            <p className="text-base-content mb-0">
              Relations to {targetClient.value!.firstName}{" "}
              {targetClient!.value!.lastName}
            </p>
            <ul className="mt-0">
              {typesFromTarget().map((type) => (
                <li key={type} className="my-0">
                  {type}
                </li>
              ))}
            </ul>
        ...
        <div className="modal-action">
          ...
          <button
            className={`btn btn-success ${submitted ? "loading" : ""}`}
            type="submit"
            disabled={
              [targetClient, relationType].some((value) => !value.valid) ||
              submitted
            }
          >
            Relate!
...
const RelationsPerClient = observer(
  ({ relations }: { relations: ClientRelation[] }) => {
    ...
    return (
      <div className="flex flex-col space-y-2">
        <div className="sticky top-0 w-full bg-base-200 rounded-btn p-2 flex flex-row items-center space-x-2">
          <div
            className="tooltip tooltip-accent tooltip-right w-fit cursor-help my-auto"
          ...
          <div className="flex flex-col">
            <p className="font-medium text-lg">
              {destClient.firstName} {destClient.lastName},{" "}
              {destClient.birthdate.toLocaleDateString()}
            </p>
            <p>
              {destClient.mobileNumber},{" "}
              <span className="text-primary">{destClient.email}</span>
            </p>
          </div>
        </div>
        <div className="grid grid-cols-3 px-2 justify-items-center gap-1">
          {relations.map((relation) => (
            <Relation key={relation.record.join()} relation={relation} />
          ))}
        </div>
      </div>
    );
...
const Relation = observer(({ relation }: { relation: ClientRelation }) => {
  const store = useStore();
  const relations = store.clientStore.relationStore;
  const mutability = useLocalObservable(() => ({
    ...
    changeValue: new ValidatableFieldData(relation.relationType, (value) => {
      const outgoingRelations = relations.indexedByFrom.get(
        relation.from.record.join(),
      );
      const otherTypes =
        outgoingRelations === undefined
          ? []
          : outgoingRelations
              .filter(
                (outgoingRelation) => outgoingRelation.dest == relation.dest,
              )
              .map((relation) => relation.relationType);
  ...
  const submitChange = async () => {
    ...
    const result = await updateClientRelation(
      relation,
      mutability.changeValue.value,
    );
    if (result.error) {
      notification.error(
        {
    ...
    ({ close }: { close: (all?: boolean) => void }) => {
      return (
        <div className="modal-box">
          ...
          <div className="modal-action">
            <button className="btn" onClick={(e) => close()}>
              Abort
            </button>
            <button
  ...
  const ChangeDialog = observer(
    ({ close }: { close: (all?: boolean) => void }) => {
      ...
      const filteredRelationTypes =
        mutability.changeValue.value.trim().length < 1
          ? potentialRelationTypes
          : potentialRelationTypes.filter((relation) => {
              return mutability.changeValue.value
                .trim()
                .split(" ")
                .flatMap((item) => item.split(","))
                .every((segment) => {
      ...
      return (
        ...
        >
          <h3 className="font-bold text-lg">Editing relation type...</h3>
          <ValidatableComboBox
            data={mutability.changeValue}
            label="Relation type"
  ...
  return (
    <div className="flex flex-row">
      ...
      <button
        className={`btn btn-square btn-xs btn-ghost text-error ${mutability.deleteSubmitted ? "loading" : ""}`}
