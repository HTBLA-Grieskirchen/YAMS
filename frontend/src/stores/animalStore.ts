...
export default class AnimalStore {
  ...
  private syncData() {
    ...
    runInAction(() => {
      ...
      Array.from(this.indexedRaces.keys())
        .filter((id) => !raceIDs.has(id))
        .forEach((id) => this.indexedRaces.delete(id));
      ...
      Array.from(this.indexedAnimals.keys())
        .filter((id) => !animalIDs.has(id))
        .forEach((id) => this.indexedAnimals.delete(id));
    });
  }
}
