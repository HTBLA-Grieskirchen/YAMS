...
export default class EventStore {
  ...
  private syncData() {
    ...
    runInAction(() => {
      ...
      Array.from(this.indexedSeminars.keys())
        .filter((id) => !seminarsIDs.has(id))
        .forEach((id) => this.indexedSeminars.delete(id));

      // Update events
      if (!(resultEvents.length > 0 && resultEvents[0].result)) return;
      const eventIDs: Set<string> = new Set(
        resultEvents[0].result
          .map((item: any) => {
      ...
      Array.from(this.indexedEvents.keys())
        .filter((id) => !eventIDs.has(id))
        .forEach((id) => this.indexedEvents.delete(id));
    });
  }
}
